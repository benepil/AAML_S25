...
class SnakeGameEnv:
    ...
    def generate_food(self):
        """
        Generates a food position that is not occupied by the snake.
        """
        while True:
    ...
    def get_grid_state(self):
        ...
        - Channel 0: Snake head (1 where the head is, 0 elsewhere)
    ...
    def step(self, action):
        ...
        # Increase the scaling factors for distance and survival rewards.
        distance_reward = 0.3 * (prev_distance - new_distance)
        survival_reward = 0.5  # you already increased to 0.5; consider even higher if needed

        self.snake_pos.insert(0, new_head)
        if new_head == self.food:
            self.score += 1
            # Increase the base food reward significantly.
            reward = 20 + distance_reward + survival_reward
            self.food = self.generate_food()
        else:
            self.snake_pos.pop()
            reward = distance_reward + survival_reward
