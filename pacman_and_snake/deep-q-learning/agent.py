...
from game_snake import SnakeGameAI
from game_pacman import PacmanGameAI
...
class Agent:
    ...
    def get_state_pacman(self, game):
        map_data = np.zeros((21, 19), dtype=np.float32)  # [rows][columns] = [height][width]

        for y in range(21):         # y = row
            for x in range(19):     # x = column
                if game.maze[y][x] == '0':
                    map_data[y][x] = 1

        #pellets
        for (x, y) in game.pellets.pellets:
            map_data[y][x] = 3

        #energizer
        for (x, y) in game.pellets.energizers:
            map_data[y][x] = 5
        ...
        for ghost in game.ghosts:
            if ghost.frightened:
                if 0 <= ghost.x < 19 and 0 <= ghost.y < 21:
                    map_data[ghost.y][ghost.x] = 6.0
            else:
                if 0 <= ghost.x < 19 and 0 <= ghost.y < 21:
                    map_data[ghost.y][ghost.x] = 4.0

        #pacman
        if 0 <= game.pacman.x < 19 and 0 <= game.pacman.y < 21:
            map_data[game.pacman.y][game.pacman.x] = 2.0
        # Normalize and reshape:
        map_data /= 5.0
        grid = map_data[np.newaxis, np.newaxis, :, :]
...
def train(model=None):
    ...
    while x < ITERATIONS:
        ...
        # get old state
        if snake:
            state_old = agent.get_state_snake(game)
        else: 
            state_old = agent.get_state_pacman(game)
        ...
        if snake:
            state_new = agent.get_state_snake(game)
        else: 
            state_new = agent.get_state_pacman(game)
        ...
        if done:
            ...
            if agent.n_games % 500 == 0:
                ...
                plot(plot_scores, plot_mean_scores, store=True)
